        ...
        layout = [
            ...
            [
                sg.Column([
                    ...
                    [
                        sg.Radio('Crossed', 'margin', default=(not self.conf_dict["Isolated"]), key="Crossed", size=(10, 1)),
                        sg.Radio('Isolated', 'margin', key="Isolated", size=(10, 1))
                    ],
                    [
            ...
            while True:
                ...
                if not values["min_rate"]:                                              # Set the rate value 5 ~ 59 sec
                    ...
                    if interval < 60:
                        ...
                        interval = 300
                        print("Warning! Please input 1 ~ 5 min value.")
                window["rate"].update(interval)                                         # update rate value
            ...
            if event == "-balance-":
                ...
                for c in self.currencies:                                                   # Read current currency item
                    if c in values[event].keys():
                        ...
                        cur_currencies.append(float(values[event][c]))
                
                # Calculate rate of change
                if len(pre_currencies) != 0:
                    percent = (np.array(cur_currencies) - np.array(pre_currencies))/np.array(pre_currencies) * 100
                    for i, c in enumerate(self.currencies):                                                   # Read current currency item
                        if c in values[event].keys():
                            window[c + str(i)].update("{}%".format(round(percent[i], 5)))                  # Update it's value with the content of new data
                # Save current currencies
                pre_currencies = cur_currencies.copy()
